...
# Define the HTTP client with session ID
client = httpx.Client(
    headers={
    ...
    cookies={
        "sessionid": "52748322597%3A9XVa0pvrO9tqe6%3A22%3AAYfAHnYf_G8iEJZDR8oKNh_BRTZdkGs_Gs5LqmsD4FE"  # Replace with your actual session ID
    }
)
...
def main():
    ...
    if app_mode == "Instagram Scraper":
        ...
        # 3. Batch Scraper Option (Scrape multiple profiles at once)
            ...
            # Streamlit UI for Batch Scraper
            if st.button("Start Batch Scraping"):
                if usernames_input:
                    with st.spinner("Starting batch scraping..."):
                        successful, failed = batch_scrape_usernames(usernames_input, rate_limit)
